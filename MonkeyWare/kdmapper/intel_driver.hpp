...
#pragma warning(disable : 4996)
		...
		if (HIWORD(pFixed->dwFileVersionLS) >= 19041) {
			const auto NtQueryInformationAtom = reinterpret_cast<void*>(GetProcAddress(LoadLibraryA("ntdll.dll"), "NtQueryInformationAtom"));
			...
			*(uint64_t*)((&kernel_injected_jmp[0]) + 2) = kernel_function_address;
			...
			const auto NtGdiGetCOPPCompatibleOPMInformation = reinterpret_cast<void*>(GetProcAddress(LoadLibraryA("win32u.dll"), "NtGdiGetCOPPCompatibleOPMInformation"));
