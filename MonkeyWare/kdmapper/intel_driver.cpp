...
HANDLE intel_driver::Load()
{
	...
	{
		std::cout << " > Failed to get Temporary Path!?!" << std::endl;
		return nullptr;
	}
	...
	{
		std::cout << " ? Vuln Driver File Failed to be Created!" << std::endl;
		return nullptr;
	}

	if (!service::RegisterAndStart(driver_path))
	{
		std::cout << " ? Failed to Register and Begin Vuln Driver!" << std::endl;
		std::remove(driver_path.c_str());
		return nullptr;
	}
...
{
	std::cout << " ? Unloading Vuln Driver.." << std::endl;
...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size)
{
	...
	{
		std::cout << "> failed to translate 0x" << reinterpret_cast<void*>(address) << std::endl;
		return false;
	}
